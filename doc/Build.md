...
# crosstool-ng version 1.23 or higher
$ brew install crosstool-ng
```

## 2. Creating a Case-sensitive File System

 Use `diskutil` to create an 8 GB blank sparse image making sure you select case-sensitive file system.
 
 The Script is in `scripts folder`.
 ```
#!/bin/bash
...
cd /Volumes/$ImageName
 ...
 ## 3. Build Toolchain
 The above script mounts file system image in `/Volumes`
 ```
...
$ cd build
 ...
 ## 4. Setting the Toolchain Options
 i586.config: for standard ABI, for example for Galileo and qemu_x86
 
 ```
 $ cp ./scripts/.config  /Volumes/CrossToolNG/build/.config
 ```
 .config 
 ```
 #
...
CT_GETTEXT_V_0_19_7=y
CT_GETTEXT_VERSION="0.19.7"
CT_GMP_V_6_1_2=y
...
# CT_COMP_TOOLS_make is not set
 ...
 Build the toolchain, using:
 ```
 $ ct-ng build
 ```
 Export the following environment variables or use the target location where the toolchain was installed:
 ```
